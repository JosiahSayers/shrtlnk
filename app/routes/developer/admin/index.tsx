...
export const loader: LoaderFunction = async () => {
  return {
    ...
    users:
      await db.$queryRaw`SELECT sequential_dates.date, COUNT("User".*) AS users_created
...
ORDER BY date ASC`,
    appsCreated:
      await db.$queryRaw`SELECT sequential_dates.date, COUNT("Application".*) AS apps_created
    FROM (SELECT CURRENT_DATE - sequential_dates.date AS DATE
        FROM generate_series(0, 30) AS sequential_dates(date)) sequential_dates
...
ORDER BY date ASC`,
    shrtlnks:
      await db.$queryRaw`SELECT sequential_dates.date, COUNT("Shrtlnk".*) AS shrtlnks_created
    FROM (SELECT CURRENT_DATE - sequential_dates.date AS DATE
        FROM generate_series(0, 30) AS sequential_dates(date)) sequential_dates
...
ORDER BY date ASC`,
    shrtlnkLoads:
      await db.$queryRaw`SELECT sequential_dates.date, COUNT("ShrtlnkLoad".*) AS shrtlnks_loaded
    FROM (SELECT CURRENT_DATE - sequential_dates.date AS DATE
        FROM generate_series(0, 30) AS sequential_dates(date)) sequential_dates
